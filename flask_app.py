...
def telegram_webhook():
    ...
    if "message" in update:
        ...
        if text == "/start":
            bot.sendMessage(
                chat_id,
                "Welcome to Dosmestic Economy Bot! Your personal assistent, {}!!".format(
                    user
                ),
                parse_mode="markdown",
            )
        ...
        if text.startswith("/expenses"):
            if len(text.split(" ")) < 4:
                ...
                bot.sendMessage(
                    chat_id,
                    "Ok, I'm done!\n {} inserted as expenses".format(value),
                    parse_mode="markdown",
                )
        ...
        if text.startswith("/income"):
            ...
            bot.sendMessage(
                chat_id,
                "Well done!\n {} inserted as income!".format(value),
                parse_mode="markdown",
            )
        ...
        if text.startswith("/summary"):
            if len(text.split(" ")) >= 2:
                ...
                bot.sendMessage(
                    chat_id,
                    "*Summary by {} for moth {} and year {}*:".format(
                        param, month, year
                    ),
                    parse_mode="markdown",
                )
